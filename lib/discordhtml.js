...
client.on('message', msg => {
    ...
    for(var i = 0; i < OnMessageHandlers.length; i++){
        if(msg.author.id !== client.user.id) {
            if (OnMessageHandlers[i].hasAttribute("trigger") && msg.content === OnMessageHandlers[i].getAttribute("trigger")) {
                HandleOnMessage(OnMessageHandlers[i].children, msg);
                break;
            } else if (!OnMessageHandlers[i].hasAttribute("trigger")) {
                HandleOnMessage(OnMessageHandlers[i].children, msg);
            }
        }
    }
...
function HandleOnMessage(children, msg){
    for(var i = 0; i < children.length; i++){
        ...
        switch(node.tagName){
            case "REPLY":
                msg.reply(node.innerHTML.split("&gt;").join(">").split("\\n").join("\n"));
                break;
            case "MESSAGE":
                msg.channel.send(node.innerHTML.split("&gt;").join(">").split("\\n").join("\n"));
                break;
        }
    }
}
